...
class SemanticDataset():
    def __init__(self, dataset_path, prob_use_caption=1, image_size=512, random_flip=False):
        ...
        arr_files.sort()
        sem_files = recursively_read(rootdir=self.dataset_path, must_contain="", exts=['npx'])
        sem_files.sort()
    ...
    def __getitem__(self, index):
        ...
        # Step 1: Add a channel dimension to make it (1, 512, 512)
        image = pix_tensor.unsqueeze(0)

        # Step 2: Repeat the grayscale channel 3 times along the channel dimension
        pix_tensor = image.repeat(3, 1, 1)
